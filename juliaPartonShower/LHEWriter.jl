    ...
    for (eventno, event) in enumerate(events)
        ...
        open(infile, "w") do file
            ...
            for i in range(1, length(momenta))
                p = momenta[i]
                mass = 0
                particlestring = string(flavours[i]) * "\t" * string(status[i]) * "\t" * string(relations[i][1]) * "\t" * string(relations[i][2]) * "\t" * string(anticolours[i]) * "\t" * string(p[1]) * "\t" * string(p[2]) * "\t" * string(p[3]) * "\t" * string(mass) * "\t0\t"* string(helicities[i]) * "\n" 
